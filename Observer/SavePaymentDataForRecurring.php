...
class SavePaymentDataForRecurring implements ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        if (preg_match('/novalnet/', $paymentMethodCode)) {
            ...
            if (!empty($paymentRef['token']) && empty($token)) {
                $profileData->setToken($paymentRef['token']);
            }
            $recurringData = [];
            ...
            if ($methodSession->getData($paymentMethodCode . '_due_date')) {
                $recurringData['due_date'] =
                $methodSession->getData($paymentMethodCode . '_due_date');
            }
