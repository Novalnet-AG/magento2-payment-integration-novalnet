    ...
    function (
        ...
        var novalnetPay = {
            ...
            initialize: function () {
                ...
                quote.totals.subscribe(function (newTotals) {
                    ...
                    const currentAmount = Math.round(
                        parseFloat(newTotals.base_grand_total) * 100
                    );
                    ...
                    if (!initialized) {
                        lastUpdatedAmount = currentAmount;
                        initialized = true;
                        self.updatePaymentForm();
                        return;
                    }
                    if (currentAmount === lastUpdatedAmount) {
                        return;
                    }
            ...
            initPaymentForm : function() {
                const self = this,
                    ...
                    initForm: {
                        ...
                        uncheckPayments: window.checkoutConfig.payment.novalnetPay.selectedMethod ? true : false,
                        showButton: false,
                    }
                };

                window?.nnPaymentInstanceInitiate == undefined && self.paymentFormInstance.initiate(paymentFormRequestObj);
                window.nnPaymentInstanceInitiate = true;

                self.paymentFormInstance.validationResponse( (data) => {
                    self.stopLoader();
                    if (window.checkoutConfig.payment.novalnetPay.selectedMethod) {
                        if (window.checkoutConfig.payment.novalnetPay.selectedMethod == "GOOGLEPAY" || window.checkoutConfig.payment.novalnetPay.selectedMethod == "APPLEPAY") {
            ...
            updatePaymentForm: function () {
	        	...
	        	var baseGrandTotal = self.getGrandTotal();
                let updatedData = {
            ...
            initPaymentFormCallbacks: function () {
                ...
                self.paymentFormInstance.walletResponse({
                    ...
                    onPaymentButtonClicked: async (setButtonValidationStatus) => {
                        var agreements = $('.payment-method._active').find('input[id^="agreement_novalnetPay_"]:visible');
                        if(agreements.length){
                            var agreementsChecked = true;
                             ...
                             if(!agreementsChecked){
                                globalMessageList.addErrorMessage({
                                    message: $t('Please accept all terms & conditions.')
                                });
                                window.scrollTo({top: 0, behavior: 'smooth'});
                                setButtonValidationStatus('FAILURE'); 
