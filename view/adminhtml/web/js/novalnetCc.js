...
define(
    ...
    function ($, alert, $t) {
        ...
        $.widget('mage.novalnetCcFormJs', {
             ...
             setButtonAttr: function  () {
                ...
                $('#submit_order_top_button, .order-totals .actions button')
                    ...
                    .on('click', function (event) {
                        if (
                            $('input[name="payment[method]"]:checked').val() === 'novalnetCc'
                        ) {
                            event.preventDefault();
                            if ($('#nn_chk_button').val() != '1') {
                                $('#nn_chk_button').val(1);
                                    getHash();
                            }
            ...
            _create:function () {
                var self = this;
                var nnCreditCardRadioBtn = $('#p_method_novalnetCc');
                nnCreditCardRadioBtn.on('change', function() {
                    if (nnCreditCardRadioBtn.is(':checked')) {
                        NovalnetUtility.setCreditCardFormHeight();
                        document.getElementById('submit_order_top_button').onclick = null;
                        document.querySelector('.action-default.scalable.save.primary').onclick = null;
                    }
                });
