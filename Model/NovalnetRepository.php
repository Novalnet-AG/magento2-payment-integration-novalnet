...
class NovalnetRepository implements \Novalnet\Payment\Api\NovalnetRepositoryInterface
{
    ...
    public function buildPayBylinkRequest($quoteId, $isAdmin = false)
    {
        ...
        $currentTheme = in_array($this->novalnetConfig->getHyvaCheckoutConfig('general/checkout'), ['onepage','mobile','default']) ? 'magento_hyva' : '';
        ...
        $data['transaction'] = [
            ...
            'system_version'   => $this->novalnetHelper->getMagentoVersion() . '-NN' .
                                    $this->novalnetHelper->getNovalnetVersion() . '-NNT'. $currentTheme
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            if (!empty($additionalData['period_unit']) && !empty($additionalData['billing_frequency'])) {
                $data['hosted_page']['display_payments_mode'] = ['SUBSCRIPTION'];
                break;
            }
