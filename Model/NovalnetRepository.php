...
class NovalnetRepository implements \Novalnet\Payment\Api\NovalnetRepositoryInterface
{
    ...
    /**
     * To get payment iframe source linkquoteFactory
     *
     * @api
     * @param mixed $quoteId
     * @return mixed
     */
    public function getPayByLink($quoteId)
    {
    ...
    public function buildPayBylinkRequest($quoteId, $isAdmin = false)
    {
        ...
        $this->clientFactory->post(NNConfig::NOVALNET_SEAMLESS_PAYMENT_URL, $this->jsonHelper->jsonEncode($data));
    ...
    public function placeOrder($data, $paymentPage = false)
    {
        try {
            ...
            $walletToken = $data['booking_details']['wallet_token'];
            ...
            $paymentData = [
                ...
                'additional_data' => [
                    ConfigProvider::NOVALNET_PAY . '_payment_data' => $this->jsonHelper->jsonEncode($data)
                ]
            ];

            if (!empty($data['booking_details']['doRedirect'])) {
                $paymentData['additional_data'][ConfigProvider::NOVALNET_PAY . '_do_redirect'] = $data['booking_details']['doRedirect'];
            }
            ...
            $payment = $order->getPayment();
            $additionalData = (!empty($payment->getAdditionalData())) ? $this->jsonHelper->jsonDecode($payment->getAdditionalData()) : [];
            if ($data['payment_details']['type'] == 'GOOGLEPAY') {
                $additionalData['NnGpaysuccesstext'] = " (" . $data['booking_details']['paymentData']['cardBrand'] . " **** ". $data['booking_details']['paymentData']['lastFour'] .")";
            } elseif ($data['payment_details']['type'] == 'APPLEPAY') {
                $additionalData['NnApplepaysuccesstext'] = " (" . $data['booking_details']['paymentData']['cardBrand'] . " **** ". $data['booking_details']['paymentData']['lastFour'] .")";
            }
