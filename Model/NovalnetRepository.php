    ...
    public function buildPayBylinkRequest($quoteId, $isAdmin = false)
    {
        ...
        $data['transaction'] = [
            ...
            'system_version'   => $this->novalnetHelper->getMagentoVersion() . '-NN' .
                                    $this->novalnetHelper->getNovalnetVersion() . '-NNT'
        ];
        ...
        foreach ($quote->getAllItems() as $item) {
            ...
            if (!empty($additionalData['period_unit']) && !empty($additionalData['billing_frequency'])) {
                if($quote->getBaseGrandTotal() <= 0 || $item->getPrice() <= 0) {
                    $data['hosted_page']['display_payments'] = $this->novalnetConfig->getRecurringPaymentTypes();
                } else {
                $data['hosted_page']['display_payments'] = $this->novalnetConfig->getRecurringPaymentTypesBasedonCondition();
                }
                break;
            }
        ...
        if ($isAdmin) {
            ...
            $data['hosted_page']['display_payments_mode'] = ['BACKEND'];
        }
