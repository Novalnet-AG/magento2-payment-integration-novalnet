...
class NNRepository implements \Novalnet\Payment\Api\NNRepositoryInterface
{
    ...
    private function creditProcess()
    {
        ...
        if (in_array(
            ...
            $updatedAmount = str_replace(',', '.', $this->currencyModel->format($additionalData['NnAmount'], ['display'=>\Zend_Currency::NO_SYMBOL], false));
    ...
    public function placeOrder($data, $paymentType, $paymentPage = false)
    {
        try {
            ...
            $walletToken = $data['transaction']['token'];
            $billingAddress = $data['order']['billing']['contact'];

            if (empty($billingAddress['email']) && !empty($data['order']['shipping']['contact']['email'])) {
                $billingAddress['email'] = $data['order']['shipping']['contact']['email'];
            }

            if (empty($billingAddress['phoneNumber']) && !empty($data['order']['shipping']['contact']['phoneNumber'])) {
                $billingAddress['phoneNumber'] = $data['order']['shipping']['contact']['phoneNumber'];
            }
            ...
            if (!$paymentPage && !$quote->isVirtual()) {
                $shippingAddress = $data['order']['shipping']['contact'];
                ...
                $shippingIdentifier = $data['order']['shipping']['method']['identifier'];
            ...
            $paymentData = [
                'method' => $paymentType,
                'additional_data' => [
                     $paymentType . '_wallet_token' => $walletToken
                 ]
            ];

            if (!empty($data['transaction']['doRedirect'])) {
                $paymentData['additional_data'][$paymentType . '_do_redirect'] = $data['transaction']['doRedirect'];
            }
            ...
            if ($paymentType == ConfigProvider::NOVALNET_GOOGLEPAY) {
                $additionalData['NnGpaysuccesstext'] = " (" . $data['transaction']['paymentData']['cardBrand'] . " **** ". $data['transaction']['paymentData']['lastFour'] .")";
            } elseif ($paymentType == ConfigProvider::NOVALNET_APPLEPAY) {
                $additionalData['NnApplepaysuccesstext'] = " (" . $data['transaction']['paymentData']['cardBrand'] . " **** ". $data['transaction']['paymentData']['lastFour'] .")";
            }
    ...
    public function getFormattedAddress($info)
    {
        ...
        return [
            ...
            'street' => !empty($info['address1']) ? $info['address1'] : 'Unspecified Street',
