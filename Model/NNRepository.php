...
class NNRepository implements \Novalnet\Payment\Api\NNRepositoryInterface
{
    ...
    private function transactionCapture()
    {
        ...
        if ($this->order->canInvoice() &&
            ...
            if ($invoiceDuedate && $this->response->getData('result/status') == 'SUCCESS') {
                ...
                if ($this->code != ConfigProvider::NOVALNET_INVOICE_GUARANTEE && $this->code != ConfigProvider::NOVALNET_INVOICE) {
    ...
    private function refundProcess()
    {
        ...
        if (!$shopInvoked && !isset($additionalData['InstalmentCancel'])) {
        ...
        if ($this->order->getState() != Order::STATE_CLOSED && $this->order->canCreditmemo() && !$shopInvoked) {
            ...
            if ($totalRefunded >= $grandTotal || $refundAmount >= $grandTotal) {
                ...
                if ($this->code == ConfigProvider::NOVALNET_INVOICE_INSTALMENT || $this->code == ConfigProvider::NOVALNET_SEPA_INSTALMENT) {
    ...
    private function transactionUpdate()
    {
        ...
        if (!empty($this->response->getData('transaction/bank_details/qr_image'))) {
            $additionalData['NnQRSource'] = $this->response->getData('transaction/bank_details/qr_image');
        }
        if ($invoiceDuedate && $this->response->getData('result/status') == 'SUCCESS') {
            $formatDate = $this->timeZone->formatDate($invoiceDuedate, \IntlDateFormatter::LONG);
    ...
    private function creditProcess()
    {
        ...
        if (in_array(
            ...
            $message = __(
                'Credit has been successfully received for the TID: %1 with an amount %2 on %3. New TID: %4 for the credit.',
                $this->parentTid,
            ...
            if ( ($totalAmount < $updatedAmount) || ($totalAmount < $grandTotal) ||
    ...
    private function instalmentProcess()
    {
        ...
        if ($this->response->getData('transaction/payment_type') == 'INSTALMENT_INVOICE') {
            ...
            $note[5] = 'Payment Reference:' . $this->response->getData('transaction/tid');
            if (isset($note[6])) {
                unset($note[6]);
            }
            $additionalData['NnInvoiceComments'] = (!empty($note)) ? implode('|', $note) : '';
        ...
        $additionalData['Nninstalmentnew'][$this->eventTid]['NninstalmentnewAmount'] = $additionalData['NnAmount'];
        $this->payment->setAdditionalData($this->jsonHelper->jsonEncode($additionalData))->save();
