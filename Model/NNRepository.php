...
class NNRepository implements \Novalnet\Payment\Api\NNRepositoryInterface
{
    ...
    private function sendEmailMagento()
    {
        try {
            ...
            $templateVars = [
                ...
                'body' => (!empty($this->emailBody)) ? $this->emailBody->render() : ''
            ];
    ...
    private function sendInstalmentmail()
    {
        $this->getEmailConfig();
        $from = ['email' => $this->emailFromAddr, 'name' => $this->emailFromName];
        $mailSubject = sprintf('Instalment confirmation %s Order no: %s', $this->storeFrontendName, $this->orderNo);
        ...
        $templateVars = [
            ...
            'sepaPayment' => ((!isset($additionalData['prepaid']) || empty($additionalData['prepaid'])) && ($this->response->getData('transaction/payment_type') == 'INSTALMENT_DIRECT_DEBIT_SEPA'))
                ? __('The instalment amount for this cycle %1 %2 will be debited from your account in one - three business days.', $this->novalnetRequestHelper->getFormattedAmount($this->response->getData('instalment/cycle_amount'), 'RAW'), $this->currency) : ""
        ];
