...
class CallbackRepository implements \Novalnet\Payment\Api\CallbackRepositoryInterface
{
    ...
    private function getPaymentTypeForOrder()
    {
        ...
        if (!empty($this->payment->getAdditionalData())) {
            $additionalData = $this->novalnetHelper->isSerialized($this->payment->getAdditionalData())
    ...
    private function transactionCapture()
    {
        ...
        if ($this->order->canInvoice() &&
            in_array($transactionStatus, ['ON_HOLD', 'PENDINsetTemplateIdentifierG'])
        ) {
    ...
    private function creditProcess()
    {
        ...
        if (in_array(
            ...
            $updatedAmount = $this->novalnetHelper->getFormattedAmount($updatedAmount);
    ...
    private function instalmentCancelProcess()
    {
        ...
        $additionalData = (!empty($this->payment->getAdditionalData())) ? ($this->novalnetHelper->isSerialized($this->payment->getAdditionalData())
                ? $this->serializer->unserialize($this->payment->getAdditionalData())
                : json_decode($this->payment->getAdditionalData(), true)) : [] ;
    ...
    private function sendInstalmentmail()
    {
        ...
        if (!empty($additionalData)) {
            $additionalData = $this->novalnetHelper->isSerialized($additionalData)
