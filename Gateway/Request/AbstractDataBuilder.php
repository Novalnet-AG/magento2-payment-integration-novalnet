...
class AbstractDataBuilder implements BuilderInterface
{
    ...
    public function buildTransactionParams($order, $paymentMethodCode)
    {
        ...
        $paymentDatas = ['token', 'pan_hash', 'unique_id', 'iban', 'wallet_token', 'bic', 'account_holder'];

        foreach ($paymentDatas as $paymentData) {
            $paymentDataValue = $methodSession->getData($paymentMethodCode . '_' . $paymentData);
            if ($paymentDataValue) {
                $data['transaction']['payment_data'][$paymentData] =
                    ($paymentData === 'account_holder') ? $paymentDataValue : preg_replace('/\s+/', '', $paymentDataValue);
            }
        }
        ...
        $data['cart_info'] = [];
            ...
            foreach ($quote->getAllVisibleItems() as $item) {
                ...
                array_push($lineItems, [
                    ...
                    'description' => '',
                    'name' => $item->getName(),
                    'price' => $this->novalnetRequestHelper->getFormattedAmount($this->taxConfig->priceIncludesTax() ? $item->getPriceInclTax() : $item->getPrice()),
                    'quantity' => $item->getQty()
                ]);

                $additionalData = $item->getAdditionalData() ? json_decode($item->getAdditionalData(), true) : [];
                if (!empty($additionalData['initial_fees'])) {
                    array_push($lineItems, [
                        'category' => '',
                        'description' => '',
                        'name' => 'Initial Fees',
                        'price' => $this->novalnetRequestHelper->getFormattedAmount($additionalData['initial_fees']),
                        'quantity' => $item->getQty()
                    ]);
                }
            }
