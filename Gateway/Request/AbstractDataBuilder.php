    ...
    public function buildTransactionParams($order, $paymentMethodCode)
    {
        ...
        $data['transaction'] = [
            ...
            'system_version'   => $this->novalnetHelper->getMagentoVersion() . '-' .
                                    $this->novalnetHelper->getNovalnetVersion() . '-' . $this->novalnetHelper->getHyvaRequestData('hyvaVersion'),
    ...
    public function buildCartInfoParams()
    {
        ...
        foreach ($quote->getAllVisibleItems() as $item) {
            array_push($lineItems, [
                ...
                'description' => '',
                'name' => $item->getName(),
                'price' => $this->novalnetHelper->getFormattedAmount($this->taxConfig->priceIncludesTax() ? $item->getPriceInclTax() : $item->getPrice()),
            ...
            if(!empty($additionalData['initial_fees'])) {
                array_push($lineItems, [
                    ...
                    'description' => '',
                    'name' => 'Initial Fees',
                    'price' => $this->novalnetHelper->getFormattedAmount($additionalData['initial_fees']),
                    'quantity' => $item->getQty()
                ]);
            }
        }
