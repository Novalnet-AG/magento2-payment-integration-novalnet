...
class AbstractDataBuilder implements BuilderInterface
{
    ...
    public function buildTransactionParams($order, $paymentMethodCode)
    {
        ...
        $paymentDatas = ['token', 'pan_hash', 'unique_id', 'iban', 'create_token'];

        foreach ($paymentDatas as $paymentData) {
            if ($methodSession->getData($paymentMethodCode . '_' . $paymentData)) {
                if ($paymentData == 'create_token' && $this->novalnetConfig->getPaymentConfig($paymentMethodCode, 'shop_type') && empty($methodSession->getData($paymentMethodCode . '_token'))) {
                    $data['transaction'][$paymentData] = $methodSession->getData($paymentMethodCode . '_' . $paymentData);
                } elseif ($paymentData == 'iban') {
                    $data['transaction']['payment_data'][$paymentData] = preg_replace('/\s+/', '', $methodSession->getData($paymentMethodCode . '_' . $paymentData));
                } else {
                    $data['transaction']['payment_data'][$paymentData] = $methodSession->getData($paymentMethodCode . '_' . $paymentData);
                }
            }
        }
