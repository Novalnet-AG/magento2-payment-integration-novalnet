...
class AbstractDataBuilder implements BuilderInterface
{
    ...
    public function build(array $buildSubject)
    {
        ...
        $cartInfoParams = $this->buildCartInfoParams($order, $paymentMethodCode);
     ...
     * @return array
     */
    public function buildCartInfoParams($order, $paymentMethodCode)
    {
        $data['cart_info'] = [];
        if ($paymentMethodCode == ConfigProvider::NOVALNET_PAYPAL) {
            $quote = $this->cart->getQuote();
            $discount = ($order->getBaseDiscountAmount()) ? str_replace('-', '', $order->getBaseDiscountAmount()) : 0;
            ...
            $data['cart_info'] = [
                ...
                'items_tax_price' => ($order->getBaseTaxAmount() > 0) ? $this->novalnetRequestHelper->getFormattedAmount($order->getBaseTaxAmount()) : 0
            ];
