...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function buildAdditionalData($response, $payment)
    {
        ...
        $dataToStore = ['transaction/test_mode', 'transaction/status', 'transaction/tid', 'custom/lang', 'transaction/due_date', 'transaction/partner_payment_reference', 'transaction/payment_data/cc_3d', 'transaction/checkout_js', 'transaction/checkout_token' , 'transaction/service_supplier_id'];
        ...
        if ($response->getData('transaction/bank_details') && $response->getData('transaction/amount') > 0) {
            $additionalData['NnInvoiceComments'] = $this->getInvoiceComments($response);
            if (!empty($response->getData('transaction/bank_details/qr_image'))) {
                $additionalData['NnQRSource'] = $response->getData('transaction/bank_details/qr_image');
            }
        }
    ...
    public function getInvoiceComments($response)
    {
        ...
        $invoicePaymentsNote .= '|Place: ' . $response->getData('transaction/bank_details/bank_place');

        $invoicePaymentsNote .= '|Please use any of the following payment references when transferring the amount. This is necessary to match it with your corresponding order:';
    ...
    */
    ...
    /**
     * This method checks whether the Hyvä checkout configuration is enabled
     * and returns specific information depending on the requested parameter.
     *
     * @param string|null $request  The type of data requested.
     *
     * @return string  The corresponding Hyvä data or an empty string if unavailable.
     */
    public function getHyvaRequestData($request)
    {
        $checkoutConfig = $this->novalnetConfig->getHyvaCheckoutConfig('general/checkout');

        if (empty($checkoutConfig) || !in_array($checkoutConfig, ['onepage', 'mobile', 'default'], true)) {
            return '';
        }

        switch ($request) {
            case 'currentTheme':
                $packageName = 'hyva-themes/magento2-default-theme';
        
                if (class_exists(InstalledVersions::class)
                    && InstalledVersions::isInstalled($packageName)
                ) {
                    $fullVersion = \Composer\InstalledVersions::getPrettyVersion($packageName);
                    $version = explode('.', $fullVersion);
                    $majorMinorVersion = $version[0] . '.' . ($version[1] ?? '0');
                    return 'magento_hyva-' . $majorMinorVersion;
                }
            case 'hyvaVersion':
                $moduleInfo = $this->moduleList->getOne('Novalnet_HyvaCheckout');
