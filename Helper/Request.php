...
class Request extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function isPageEnabledForApplePay($page)
    {
        ...
        $enabledPages = $this->novalnetConfig->getPaymentConfig(ConfigProvider::NOVALNET_APPLEPAY, 'enabled_pages');
        $enabledPages = (!empty($enabledPages)) ? explode(',', $enabledPages) : [];
        return $isApplePayEnabled && in_array($page, $enabledPages) && $this->canUseForGuestCheckout();
    }
