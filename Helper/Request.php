...
class Request extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function buildAdditionalData($response, $payment)
    {
        ...
        $dataToStore = ['transaction/test_mode', 'transaction/status', 'transaction/tid', 'custom/lang', 'transaction/due_date', 'transaction/partner_payment_reference','transaction/service_supplier_id', 'transaction/payment_data/cc_3d', 'transaction/checkout_js', 'transaction/checkout_token'];
    ...
    public function savePaymentToken($order, $paymentMethodCode, $response)
    {
        ...
        if ($this->novalnetConfig->isOneClickPayment($paymentMethodCode)) {
            ...
            foreach ($order->getItems() as $item) {
                ...
                if (!empty($additionalData['period_unit']) && !empty($additionalData['billing_frequency']) && $checkoutSession->getData($paymentMethodCode)->getData($paymentMethodCode . '_create_token') == 0) {
                    $tokenInfo['NnSubscription'] = 1;
                    break;
                }
            }
